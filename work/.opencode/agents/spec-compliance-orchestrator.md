...
3. For `frame_obligations`, invoke `code-investigator` with the returned packet unchanged.
4. For `investigate`, invoke `code-investigator` with the returned packet unchanged.
5. For `review`, invoke `evidence-reviewer` with the returned review packet unchanged.
6. After each subagent finishes, call `audit_next` exactly once.
7. For `finish`, call `audit_finish`. For `done`, report the output. For `blocked`, stop and report the reason.
